...
class ctrlPID:
    def __init__(self):
        ...
        self.integrator_theta = 0.  # integrator for theta
        self.integrator_psi = 0.  # integrator for psi
    ...
    def update(self, r, y):
        ...
        # update integrators if movement speed is small
        if np.abs(self.theta_dot) < 0.05:
            self.integrator_theta = self.integrator_theta + (P.Ts / 2) * (error_theta + self.error_theta_prev)
        if np.abs(self.psi_dot) < 0.05:
            self.integrator_psi = self.integrator_psi + (P.Ts / 2) * (error_psi + self.error_psi_prev)

        # pitch control - if you choose to use the F_e only instead, that is acceptable too
        force_unsat = force_equilibrium + (self.kp_pitch * error_theta) - (self.kd_pitch * self.theta_dot) + (self.integrator_theta * self.ki_pitch)
        force = saturate(force_unsat, -P.force_max, P.force_max)

        # outer loop yaw control
        phi_ref_unsat = self.kp_yaw * error_psi - self.kd_yaw * self.psi_dot + self.integrator_psi * self.ki_yaw
