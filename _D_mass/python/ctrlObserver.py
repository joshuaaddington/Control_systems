import numpy as np
import control as cnt
import massParam as P

class ctrlObserver:
    def __init__(self):
        #--------------------------------------------------
        # State Feedback Control Design
        #--------------------------------------------------
        #  tuning parameters
        tr = 1.0
        zeta = 0.707 # TODO could be higher, but then would need to calculate omega_n differently
        integrator_pole = [-2.0]
        tr_obs = tr/10.0  # rise time for observer
        zeta_obs = 0.90  # damping ratio for observer

        # State Space Equations
        # xdot = A*x + B*u
        # y = C*x
        self.A = np.array([[0.0, 1.0],
                           [-P.k / P.m, -P.b / P.m]])
        self.B = np.array([[0.0],
                           [1.0 / P.m]])
        self.C = np.array([[1.0, 0.0]])

        # form augmented system
        Cr = self.C # output matrix for controlling z
        A1 = np.zeros((3, 3))
        A1[0:2, 0:2] = self.A
        A1[2, 0:2] = -Cr
        
        B1 = np.zeros((3, 1))
        B1[0:2, :] = self.B
        
        # gain calculation
        wn = 2.2 / tr
        des_char_poly = np.convolve(
            [1, 2*zeta*wn, wn**2],
            np.poly(integrator_pole)
            )
        des_poles = np.roots(des_char_poly)

        # Compute the gains if the system is controllable
        if np.linalg.matrix_rank(cnt.ctrb(A1, B1)) != A1.shape[0]:
            print("The system is not controllable")
        else:
            K1 = cnt.place(A1, B1, des_poles)
            self.K = K1[0][0:2]
            self.ki = K1[0][2]

        # observer design
        wn_obs = 0.5*np.pi/(tr_obs*np.sqrt(1-zeta_obs**2)) # natural frequency for when zeta is not equal to 0.707
        des_obsv_char_poly = [1, 2*zeta_obs*wn_obs, wn_obs**2]
        des_obsv_poles = np.roots(des_obsv_char_poly)

        # Compute the gains if the system is controllable
        if np.linalg.matrix_rank(cnt.ctrb( self.A.T, self.C.T)) != 2:
            print("The system is not observerable")
        else:
            self.L = cnt.acker(self.A.T, self.C.T, des_obsv_poles).T
        print('K: ', self.K)
        print('ki ', self.ki)
        print('L^T: ', self.L.T)

        #--------------------------------------------------
        # variables to implement integrator
        self.integrator = 0.0  # integrator
        self.error_prev = 0.0  # error signal delayed by 1 sample
        self.x_hat = np.array([
            [0.0],
            [0.0],
        ])
        self.force_prev = 0.0          # control, delayed by one sample

    def update(self, z_r, y):
        # update the observer and extract z_hat
        x_hat = self.update_observer(y)
        z_hat = x_hat[0][0]

        # integrate error
        error = z_r - z_hat
        self.integrator = self.integrator \
                          + (P.Ts / 2.0) * (error + self.error_prev)
        self.error_prev = error

        # Compute the state feedback controller
        force_unsat = -self.K @ x_hat - self.ki*self.integrator

        # compute saturated force
        force = saturate(force_unsat[0], P.F_max)

        # TODO not clear if this type of anti-windup is necessary
        # integrator anti-windup
        if self.ki != 0.0:
            self.integrator = self.integrator + P.Ts/self.ki*(force-force_unsat)
        self.force_prev = force
        
        return force, x_hat

    def update_observer(self, y_m):
        # update the observer using RK4 integration
        F1 = self.observer_f(self.x_hat, y_m)
        F2 = self.observer_f(self.x_hat + P.Ts / 2 * F1, y_m)
        F3 = self.observer_f(self.x_hat + P.Ts / 2 * F2, y_m)
        F4 = self.observer_f(self.x_hat + P.Ts * F3, y_m)
        self.x_hat += P.Ts / 6 * (F1 + 2 * F2 + 2 * F3 + F4)
        return self.x_hat

    def observer_f(self, x_hat, y_m):
        # xhatdot = A*xhat + B*(u-ue) + L(y-C*xhat)
        xhat_dot = self.A @ x_hat \
                   + self.B * self.force_prev \
                   + self.L @ (y_m - self.C @ x_hat)
        return xhat_dot


def saturate(u, limit):
    if abs(u) > limit:
        u = limit * np.sign(u)
    return u


