...
Theta_over_F = transfer_func_partC[1]

params = [(m1, P.m1), (m2, P.m2), (ell, P.ell), (g, P.g), (ze, P.ze)]
Theta_over_F = Theta_over_F.subs(params)

poles = sp.solve(Theta_over_F.as_numer_denom()[1], s)
...
Z_over_F_num = Theta_over_F.as_numer_denom()[0]
Z_over_F_den = Theta_over_F.as_numer_denom()[1]
...
# compute PD gains
kd = alpha1 * ((P.ell**2 + 3*P.m1*(P.ell/2)**2)/3*P.ell)
kp = alpha0 * ((P.ell**2 + 3*P.m1*(P.ell/2)**2)/3*P.ell)
# %%
